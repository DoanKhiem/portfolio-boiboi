<template>
    <div class="mx-auto py-5 px-4 gap-10">
        <div class="grid md:grid-cols-2 grid-cols-1 relative border-2 rounded-3xl mt-5 border-[#d7a947] p-5 md:col-span-3">
            <div class="" >
                <img src="~/assets/images/three-dot.png" alt="Image Avatar" class="w-16 md:w-32 absolute -top-2 md:-top-4 right-8 md:right-20">
                <img src="~/assets/images/star-red.png" alt="Image Avatar" class="w-8 md:w-16 absolute top-24 md:top-28 -left-4 md:-left-8">
                <h2
                    class="absolute -top-4 left-6 md:left-32 px-10 pacifico-regular-font text-xl md:text-3xl border-2 rounded-full border-primary bg-white">
                    Boi Boi Truong</h2>
                <h1 data-aos="zoom-in-up" class="text-primary abril-fatface-regular-font text-8xl md:text-9xl">Port</h1>
                <h1 data-aos="zoom-in-up" class="abril-fatface-regular-font text-8xl md:text-9xl">Folio</h1>
                <div class="mt-2" data-aos="zoom-in">
                    <span class="crimson-pro-font border-2 rounded-full px-4 border-primary text-2xl md:text-4xl">Communication
                        Portfolio</span>
                </div>
                <div class="mt-5">
                    <p data-aos="zoom-in">If opportunity doesn't knock, build a portfolio When it does, knock me at:</p>
                </div>
                <div class="mt-5" data-aos="zoom-in-right">
                    <div class="bg-primary inline-flex rounded-full px-5 py-1">
                        <img class="w-[26px] mr-2" src="~/assets/images/mail.png" alt="Icon Email">
                        <p class="text-white ">truongboiboipy123@gmail.com</p>
                    </div>
                </div>
                <div class="mt-2" data-aos="zoom-in-right">
                    <div class="bg-primary inline-flex rounded-full px-5 py-1">
                        <img class="w-[26px] mr-2" src="~/assets/images/phone.png" alt="Icon Email">

                        <p class="text-white">(+84) 905146898</p>
                    </div>
                </div>
                <div class="mt-2" data-aos="zoom-in-right">
                    <div class="bg-primary inline-flex rounded-full px-5 py-1">
                        <img class="w-[26px] mr-2" src="~/assets/images/address.png" alt="Icon Email">
                        <p class="text-white">Phu Yen, Vietnam</p>
                    </div>
                </div>
                <div class="mt-2" data-aos="zoom-in-right">
                    <div class="bg-primary inline-flex rounded-full px-5 py-1">
                        <img class="w-[26px] mr-2" src="~/assets/images/birthday.png" alt="Icon Email">
                        <p class="text-white">22th, June, 2007</p>
                    </div>
                </div>
            </div>
            <div class="mt-5 md:mt-0 relative" data-aos="fade-up">
                <img src="~/assets/images/star-red.png" alt="Image Avatar" class="w-8 md:w-16 absolute top-48 md:top-80 -left-4 md:-left-4 z-10">
                <img src="~/assets/images/avt-1.png" alt="Image Avatar"
                    class="w-full object-cover transition-transform duration-300 hover:scale-105">
            </div>
        </div>
        <!-- <div class="md:col-span-2 relative bg-primary">
            <img src="~/assets/images/avt-1.png" alt="Image Avatar"
                    class="w-full  object-cover rounded-lg transition-transform duration-300 hover:scale-105">
        </div> -->
    </div>
</template>

<script setup lang="ts">

</script>

<style scoped></style>